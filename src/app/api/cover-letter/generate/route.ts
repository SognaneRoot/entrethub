...
const TONE_PROMPTS: Record<string, string> = {
  professional: 'Ton professionnel, formel et structuré.',
  enthusiastic: 'Ton enthousiaste et dynamique. Montre une vraie motivation.',
  creative: 'Ton créatif et original. Se démarque des lettres classiques.',
};
...
export async function POST(req: Request) {
  ...
  const { data: user } = await db.from('users').select('id,first_name,last_name,email').eq('clerk_id', userId).single();
  ...
  const systemPrompt = `Tu es un expert en rédaction de lettres de motivation. ${TONE_PROMPTS[tone] ?? TONE_PROMPTS.professional} Retourne UNIQUEMENT le texte de la lettre (280-380 mots), sans titre ni balises.`;
